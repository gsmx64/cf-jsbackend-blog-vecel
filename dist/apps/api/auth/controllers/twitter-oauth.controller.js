"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TwitterOAuthController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const auth_service_1 = require("../services/auth.service");
const twitter_oauth_guard_1 = require("../guards/twitter-oauth.guard");
let TwitterOAuthController = class TwitterOAuthController {
    constructor(authService) {
        this.authService = authService;
    }
    async twitterLogin() {
        return;
    }
    async twitterLoginCallback(req) {
        return this.authService.login(req.user);
    }
};
exports.TwitterOAuthController = TwitterOAuthController;
__decorate([
    (0, common_1.UseGuards)(twitter_oauth_guard_1.TwitterOauthGuard),
    (0, common_1.Get)('auth/twitter'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], TwitterOAuthController.prototype, "twitterLogin", null);
__decorate([
    (0, common_1.Get)('auth/twitter/callback'),
    (0, common_1.UseGuards)(twitter_oauth_guard_1.TwitterOauthGuard),
    __param(0, (0, common_1.Request)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], TwitterOAuthController.prototype, "twitterLoginCallback", null);
exports.TwitterOAuthController = TwitterOAuthController = __decorate([
    (0, swagger_1.ApiTags)('Auth-Twitter'),
    (0, common_1.Controller)(),
    __metadata("design:paramtypes", [auth_service_1.AuthService])
], TwitterOAuthController);
//# sourceMappingURL=twitter-oauth.controller.js.map